'''
2026.04.22 revised version 1 by jialu
安装的nnunet新版本中，以下修改已经新增，由于我的nnunet不是最新版，所以需要手动将新版使用的nnUNetDataset复制到本文件中
'''
import os
...
from batchgenerators.utilities.file_and_folder_operations import join, load_pickle, isfile, subfiles
...
class nnUNetDataset(object):
    ...
    def load_case(self, key):
        ...
        if 'open_data_file' in entry.keys():
            ...
            data = np.load(entry['data_file'][:-4] + ".npy", mmap_mode='r+')
            #original: 'r'
        ...
        if 'open_seg_file' in entry.keys():
            ...
            # print("entry['data_file'][:-4]:",entry['data_file'][:-4])
            seg = np.load(entry['data_file'][:-4] + "_seg.npy", mmap_mode='r+')
            if self.keep_files_open:
        ...
        if 'seg_from_prev_stage_file' in entry.keys():
            if isfile(entry['seg_from_prev_stage_file'][:-4] + ".npy"):
                seg_prev = np.load(entry['seg_from_prev_stage_file'][:-4] + ".npy", mmap_mode='r+')
            else:
...
class nnUNetBaseDataset(ABC):
    ...
    @staticmethod
    @abstractmethod
    def case_exists(output_filename_truncated: str) -> tuple[bool, bool, bool]:
        """
        Check if file has already been preprocessed.
        :param output_filename_truncated: The filename of the case without the file ending.
        :return: tuple[bool, bool, bool] (img_exists, seg_exists, pkl_exists)
        """
        pass
...
class nnUNetDatasetNumpy(nnUNetBaseDataset):
    ...
    def case_exists(output_filename_truncated: str) -> tuple[bool, bool, bool]:
        """
        Check if img, segmentation and pickle of the case exist.
        Img and Seg are stored in the same `.npz` for `nnUNetDatasetNumpy`.
        :param output_filename_truncated: The filename of the case without the file ending.
        :return: tuple[bool, bool, bool] (img_exists, seg_exists, pkl_exists)
        """
        img_file = output_filename_truncated + ".npz"
        pkl_file = output_filename_truncated + ".pkl"
        img_exists = isfile(img_file)
        seg_exists = img_exists
        pkl_exists = isfile(pkl_file)
        return img_exists, seg_exists, pkl_exists
...
class nnUNetDatasetBlosc2(nnUNetBaseDataset):
    ...
    def case_exists(output_filename_truncated: str) -> tuple[bool, bool, bool]:
        ...
        :return: tuple[bool, bool, bool] (img_exists, seg_exists, pkl_exists)
        """
        img_file = output_filename_truncated + ".b2nd"
        seg_file = output_filename_truncated + "_seg.b2nd"
        pkl_file = output_filename_truncated + ".pkl"
        img_exists = isfile(img_file)
        seg_exists = isfile(seg_file)
        pkl_exists = isfile(pkl_file)
        return img_exists, seg_exists, pkl_exists
