...
class WassersteinLoss(torch.nn.modules.loss._Loss):
    ...
    def _wasserstein_loss(self, prediction: Float[Tensor, "*spatial_dimensions"],
        ...
        (prediction_birth_coordinates, prediction_death_coordinates, target_birth_coordinates, target_death_coordinates) = (
            ...
            for array in [barcode_result_prediction.birth_coordinates[0], barcode_result_prediction.death_coordinates[0],
                            barcode_result_target.birth_coordinates[0], barcode_result_target.death_coordinates[0]]])
        # print(barcode_result_prediction.num_pairs)
        ...
        #     [torch.tensor(array, device=prediction.device, dtype=torch.long) if array.strides[-1] > 0 else torch.zeros(0, len(prediction.shape), device=prediction.device, dtype=torch.long)
        #     for array in [barcode_result_prediction.input1_matched_birth_coordinates[0], betti_matching_result.input1_matched_death_coordinates[0],
        ...
        for prediction_pairs_dim, target_pairs_dim in zip(
            torch.split(prediction_pairs, barcode_result_prediction.num_pairs[0].tolist()),
            torch.split(target_pairs, barcode_result_target.num_pairs[0].tolist())
        ):
